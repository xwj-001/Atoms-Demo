...
.del{background:none;color:#a1a1aa;padding:2px 6px;font-size:16px}
...
.dot{width:26px;height:26px;border-radius:50%;border:2px solid #a78bfa;flex:none;display:grid;place-items:center;color:#fff;font-size:14px;background:none;transition:.2s}
...
.x{background:none;border:0;color:#a5b4fc;font-size:20px;line-height:1}
...
.x{background:none;border:0;color:#a3a3a3}
...
.x{background:none;border:0;color:#99f6e4;font-size:20px}
.x:hover{color:#e11d48}
...
export function matchTemplate(prompt: string, style: StyleTag): TemplateMatch {
  ...
  const hit = PRESET_TEMPLATES.find((t) => t.keywords.some((k) => lower.includes(k.toLowerCase())));
  if (hit) {
