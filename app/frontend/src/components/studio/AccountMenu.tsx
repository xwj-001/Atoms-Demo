import { CloudUpload, LogIn, LogOut, RefreshCw, User2, UserRound } from 'lucide-react';
...
/** 右上角账户入口：区分正式账号与游客会话，分别提供同步与升级登录 */
        ...
        <DropdownMenuContent align="end" className="w-64">
          <DropdownMenuLabel className="space-y-1">
            ...
            <p className="text-[11px] font-normal leading-relaxed text-muted-foreground">
              作品仅保存在这台设备的本地库，登录正式账号后可同步到云端并公开分享。
            </p>
          </DropdownMenuLabel>
          <DropdownMenuSeparator />
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-60">
        <DropdownMenuLabel className="space-y-1">
          <p className="text-xs font-semibold">{user.name || user.email || '已登录'}</p>
          <p className="text-[11px] font-normal leading-relaxed text-muted-foreground">
            {lastSyncText}
          </p>
        </DropdownMenuLabel>
