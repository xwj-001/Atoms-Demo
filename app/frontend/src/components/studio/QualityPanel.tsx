...
import { BASIC_CHECK_ITEMS } from '@/lib/settings';
      ...
      <section className="rounded-xl border border-white/70 bg-white/55 p-3">
        ...
        {!report ? (
          <p className="text-[11px] leading-relaxed text-muted-foreground">
            生成完成后，这里会展示静态校验结论。只保留 {BASIC_CHECK_ITEMS.length} 项硬性检查：
            {BASIC_CHECK_ITEMS.join(' · ')}。静态校验在毫秒内完成，沙箱体检有{' '}
            {(AUDIT_BUDGET_MS / 1000).toFixed(0)} 秒硬性上限，到点即按已得结论收尾；更长的等待只来自
            「标准 / 严格」档的自动重修轮次，可在设置里切换。
          </p>
        ) : (
      ...
      <section className="rounded-xl border border-white/70 bg-white/55 p-3">
        <header className="mb-2 flex flex-wrap items-center gap-2">
          ...
          {auditMs !== null && (
            <Badge
              className={`h-5 px-1.5 text-[10px] ${
                auditMs <= AUDIT_BUDGET_MS
                  ? 'bg-mint/18 text-mint hover:bg-mint/18'
                  : 'bg-warm/18 text-warm-foreground hover:bg-warm/18'
              }`}
            >
              耗时 {(auditMs / 1000).toFixed(1)}s / 上限 {(AUDIT_BUDGET_MS / 1000).toFixed(0)}s
            </Badge>
          )}
          {smoke && (
            <Badge
        ...
        {!smoke && !visual ? (
          <p className="text-[11px] leading-relaxed text-muted-foreground">
            体检会在预览沙箱里自动填表、提交并逐个点击可交互元素，判断有没有「点了没反应」的死按钮；同时检测横向溢出、内容裁切、点击目标过小与文字对比度不足。整轮体检带{' '}
            {(AUDIT_BUDGET_MS / 1000).toFixed(0)} 秒硬性时间预算，超时即带着已查到的问题收尾，不会卡住你。切到「预览」视图即可自动跑一轮。
          </p>
        ) : (
              ...
              <div className="rounded-lg bg-white/60 px-2.5 py-2">
                ...
                {smoke.truncated && (
                  <p className="mt-1 text-[10px] leading-relaxed text-muted-foreground">
                    已达时间预算提前收尾，仅覆盖了前几个交互目标；需要更彻底可再点一次体检。
                  </p>
                )}
