...
import { atomsModelLabel } from '@/lib/settings';
import {
  FILE_LABEL,
  filesMetrics,
  isSandboxMessage,
...
export default function PreviewPane({
  ...
  const runAudit = useCallback(() => {
    ...
    setSmoke(null);
    setVisual(null);
    frame.contentWindow.postMessage(buildHostMessage('run-smoke'), '*');
    // 冒烟会改动 DOM，稍等一拍再做视觉体检，检测的是交互后的真实状态
    window.setTimeout(() => {
      frame.contentWindow?.postMessage(buildHostMessage('run-visual'), '*');
    }, 320);
    // 体检脚本异常时不至于让按钮一直转
    if (auditTimer.current) window.clearTimeout(auditTimer.current);
    auditTimer.current = window.setTimeout(() => setAuditing(false), 3000);
  }, []);
  ...
  useEffect(() => {
    ...
    // iframe 需要时间加载并执行脚本，延迟触发首次体检
    const timer = window.setTimeout(() => runAudit(), 900);
      ...
      <div className="flex flex-wrap items-center gap-1.5 px-4 py-2 text-[11px] text-muted-foreground">
        ...
        {phase === 'done' && strategy && (
          <>
            ...
            <Badge variant="outline" className="h-5 bg-white/50 px-1.5 text-[11px]">
              {usedMode === 'mock' ? '离线模板' : usedMode === 'openai' ? 'OpenAI 兼容' : 'Atoms 代理'}
            </Badge>
            {usedMode !== 'mock' && usedModel && (
              <Badge
                ...
                title={usedModel}
              >
                {usedMode === 'atoms' ? atomsModelLabel(usedModel) : usedModel}
              </Badge>
            )}
            {usage?.mode === 'patch' && (
              <Badge variant="outline" className="h-5 bg-white/50 px-1.5 text-[11px]">
