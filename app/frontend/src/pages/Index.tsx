...
import ChangePasswordDialog from '@/components/studio/ChangePasswordDialog';
...
import { syncWithCloud, type StudioUser } from '@/lib/cloud';
import {
  fetchAccount,
...
import { loadSettings, maxAttemptsOf, type LlmMode, type StudioSettings } from '@/lib/settings';
...
type SessionKind = 'loading' | 'none' | 'guest' | 'account';
...
export default function Index() {
  ...
  /** 首屏解析会话：优先自建账号令牌，其次本地游客会话，都没有则展示登录门 */
  useEffect(() => {
    let alive = true;
  ...
  const runGeneration = useCallback(
    async (options: {
      ...
      try {
        const result = await generateValidated({
          ...
          onAttempt: (record) => {
            ...
            if (!record.passed && record.attempt === 1) {
              // 快速档不会自动重修，文案要如实告知手动入口，别让用户白等
              setStatusText(
                maxAttemptsOf(settings.qualityLevel) > 1
                  ? `首轮校验有 ${record.failed.length} 项未通过，正在自动修复…`
                  : `校验有 ${record.failed.length} 项未通过，可在校验面板点「让 AI 再修一轮」`,
              );
            }
          },
        });
        ...
        // 命中预置模板时一次模型都没调用，把原因讲清楚，用户才知道为什么这次是秒出
        const speedText = result.templateName
          ? `命中预置模板「${result.templateName}」，未调用模型 · `
          : '';
        ...
        if (iterate && currentApp?.id) {
          ...
          setLocks(emptyLocks());
          setStatusText(`已保存「${created.name}」· ${speedText}${qualityText}`);
          if (result.report.passed) {
  ...
  /** 自建账号登录成功：建立会话并立即静默同步一次 */
  const handleAccountLogin = async (email: string, password: string) => {
    const account = await accountLogin(email, password);
    clearGuestSession();
    setGuest(null);
    setUser(account);
    setSessionKind('account');
    ...
    void runSync(account, true);
  };

  /** 自建账号注册成功后直接进入工作台 */
  const handleAccountRegister = async (email: string, password: string, displayName: string) => {
    const account = await accountRegister(email, password, displayName || undefined);
    clearGuestSession();
    setGuest(null);
    setUser(account);
    setSessionKind('account');
    resetWorkbench();
    toast.success(`注册成功，已登录为 ${account.name}`);
    void runSync(account, true);
  };

  /** 游客升级为正式账号：回到登录门重新走登录/注册 */
  ...
  const authState: AuthState = sessionKind === 'account' ? 'authenticated' : 'anonymous';
