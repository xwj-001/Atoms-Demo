...
class DeepSeekStatusResponse(BaseModel):
    """DeepSeek 通道可用性探测结果（不回显密钥本身）。"""

    configured: bool = Field(..., description="运行时是否已注入 DEEPSEEK_API_KEY。")
    base_url: str = Field(..., description="实际使用的上游 Base URL。")
    default_model: str = Field(..., description="未指定模型时使用的默认模型名。")